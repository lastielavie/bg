...
st.set_page_config(
    page_title="Bagi Hasil Teknisi", layout="wide", page_icon="🧰"
)
...
NAMA_TARIF_TETAP_20 = [
    ...
    "HAMZAH MAULANA",
    "DAVID SONDAKH",
    "FATHUR ROHMAN SOBARNA",
    "ALAI ARKAN",
    "ALFIN DAMARA",
    "ADI FIRDAUS",
    "M IQBAL PRADANA",
    "IRSYAD PANCA GUNAWAN",
    "EGI SETYA RAMADHANI",
    "ALVI SYAHLANI RAMADHAN",
    "M NAUFAL",
    "M HANIF FATIN",
    "SYAHDAN IBNU FAUZI",
    "JUNARA",
    "KARIM AGAKHAN",
    "FARID HASBY ASH SHIDDIQ",
    "RIDWAN KURNIAWAN",
]
...
def label_periode(bulan_gaji, tahun_gaji):
  ...
  return (
      f"Gaji {BULAN_NAMES[bulan_gaji]} {tahun_gaji}  "
...
def cocokkan_cabang(tok: str, kanonik, alias):
  ...
  for a, v in alias.items():
    ...
    if A and (A.startswith(t) or t.startswith(A)):
      return v, f"alias {a}"
  kandidat = [
      c for c in kanonik if _huruf(c).startswith(t) or t.startswith(_huruf(c))
  ]
  if len(kandidat) == 1:
    return kandidat[0], ""
  if len(kandidat) > 1:
...
def baca_mentah(items: tuple, kanonik: tuple, alias_items: tuple):
  ...
  for nama_berkas, isi in items:
    ...
    for d, bagian in potongan:
      ...
      if not cab:
        asal, ket = (
            ("kolom CABANG", "beragam per baris")
            if beragam
      ...
      catatan.append({
          ...
          "Bagian": bagian or "—",
          "Cabang": (
              cab if cab else ("(beragam)" if beragam else LABEL_TANPA_CABANG)
          ),
          "Dari": asal,
          "Catatan": ket,
          "Baris": len(d),
...
def bersihkan(df: pd.DataFrame) -> pd.DataFrame:
  ...
  df["CABANG"] = (
      ...
      .replace({
          "": LABEL_TANPA_CABANG,
          "nan": LABEL_TANPA_CABANG,
          "NaN": LABEL_TANPA_CABANG,
          "None": LABEL_TANPA_CABANG,
      })
      .fillna(LABEL_TANPA_CABANG)
  )

  fin = (
      _nama_teknisi_bersih(df["NAMA TEKNISI (FINAL)"])
      if "NAMA TEKNISI (FINAL)" in df.columns
      else pd.Series("", index=df.index)
  )
  asli = (
      _nama_teknisi_bersih(df["NAMA TEKNISI"])
...
buang_duplikat = st.sidebar.checkbox(
    "Buang kiriman ulang (duplikat)",
    value=True,
    key="opsi_dedup",
    help=(
        "Baris yang sama persis (cabang, no faktur, barang, qty, total, tanggal)"
...
try:
  if up is not None:
    ...
    if not mentah.empty and not catatan_berkas.empty:
      ...
      if perlu.any():
        st.sidebar.warning(
            f"{int(perlu.sum())} bagian belum ketahuan cabangnya — "
            "isi manual di bawah."
        )
        with st.sidebar.form("form_cabang"):
          ...
          if st.form_submit_button("Terapkan nama cabang"):
            ...
            st.rerun()
      for (berkas, bagian), cab in st.session_state.get(
          "peta_cabang", {}
      ).items():
        ...
        mc = (catatan_berkas["Berkas"] == berkas) & (
            catatan_berkas["Bagian"] == bagian
        )
    ...
    if (
        not mentah.empty
        and buang_duplikat
        and all(c in mentah.columns for c in KUNCI_DUPLIKAT)
    ):
      sebelum = len(mentah)
      mentah["__N__"] = mentah.groupby(
          KUNCI_DUPLIKAT + ["__BERKAS__"], dropna=False
      ).cumcount()
      mentah = mentah.drop_duplicates(
          subset=KUNCI_DUPLIKAT + ["__N__"], keep="last"
      ).drop(columns="__N__")
...
if not catatan_berkas.empty:
  with st.sidebar.expander("📄 Rincian berkas terbaca", expanded=True):
...
st.caption(
    ...
    f"{jasa_all.loc[jasa_all['TEKNISI'] != 'TIDAK ADA TEKNISI', 'TEKNISI'].nunique()}"
    " teknisi · "
    f"data {jasa_all['TGL'].min():%d %b %Y} – {jasa_all['TGL'].max():%d %b %Y}"
)

with st.expander(
    "⚙️ Pengaturan Tarif Bagi Hasil — klik untuk mengubah", expanded=False
):
  ...
  with k2:
    cab_kerusakan = st.multiselect(
        ...
        default=[
            c
            for c in CABANG_ACUAN_KERUSAKAN
...
urutan = [prioritas.upper()] + [
    k for k in KATA_KUNCI_TARIF if k != prioritas.upper()
]
...
if cab_kerusakan and "KERUSAKAN" in jasa_all.columns:
  ...
  if m_ker.any():
    jasa_all.loc[m_ker, "TARIF_LABEL"] = [
        label_dari_kerusakan(k, p)
        for k, p in zip(
            jasa_all.loc[m_ker, "KERUSAKAN"], jasa_all.loc[m_ker, "KAT_JUAL"]
        )
    ]
...
periode_list = daftar_periode_gaji(
    jasa_all["TGL"].min(), jasa_all["TGL"].max()
)
...
with fb:
  cab_opts = ["Semua Cabang"] + sorted(
      jasa_all["CABANG"].dropna().unique().tolist()
  )
  f_cabang = st.selectbox("Cabang", cab_opts, key="f_cabang")
with fc:
  sembunyikan = st.checkbox(
      "Sembunyikan baris tanpa nama teknisi", value=False, key="f_hide"
  )
...
if isinstance(pilih, str):
  ...
  periode_txt = (
      f"{a.day} {BULAN_NAMES[a.month]} {a.year} – "
      f"{b.day} {BULAN_NAMES[b.month]} {b.year}"
  )
  tag_file = f"gaji-{pilih[0]}-{pilih[1]:02d}"

jasa_tampil = (
    jasa[jasa["TEKNISI"] != "TIDAK ADA TEKNISI"] if sembunyikan else jasa
)
...
tanpa_nama = jasa.loc[
    jasa["TEKNISI"] == "TIDAK ADA TEKNISI", "TOTAL HARGA"
].sum()
    ...
    kpi_html([
        {
            "label": "Omzet Jasa",
            "value": rp(omzet),
            "sub": f"{len(jasa):,} baris",
            "grad": "linear-gradient(135deg,#1f3864,#2e5394)",
        },
        {
            "label": "Bagi Hasil (Aturan)",
            "value": rp(bh),
        ...
        },
        {
            ...
            "grad": (
                "linear-gradient(135deg,#e0921f,#e2b21a)"
                if selisih >= 0
        ...
        {
            "label": "Omzet Tanpa Nama Teknisi",
            "value": rp(tanpa_nama),
            "sub": (
                f"{(jasa['TEKNISI'] == 'TIDAK ADA TEKNISI').sum():,} baris"
            ),
...
rek = (
    jasa_tampil.groupby(["TEKNISI", "CABANG"], as_index=False)
    .agg(
...
if cari:
  m = rek_show.apply(
      lambda r: cari.upper() in f"{r['Nama Teknisi']} {r['Cabang']}".upper(),
      axis=1,
  )
  rek_view = rek_show[m]
...
KOLOM_POTONGAN = [
    ...
    "Potongan Kasbon",
    "Keterlambatan",
    "Potongan Minus Audit",
    "Potongan Audit Compliance",
...
KOLOM_GAJI = (
    KOLOM_POTONGAN
    + ["Total Potongan", "Gaji Teknisi", "Nett Bagi hasil"]
    + KOLOM_CADANGAN
    + ["Total Cadangan 7 Tahun"]
)
...
def rekap_kualifikasi(df, keys):
  ...
  out = base.merge(_pivot("TOTAL HARGA", "Omzet"), on=keys, how="left").merge(
      _pivot("BAGI_HASIL", "Bagi Hasil"), on=keys, how="left"
  )
  out["Selisih"] = out["BH"] - out["Flat"]
  out["Efektif %"] = (
      out["BH"] / out["Omzet"].replace(0, pd.NA) * 100
  ).round(1)

  urut = (
      list(keys)
      + ["Baris"]
      + [f"Omzet {k}" for k in KATEGORI_ORDER]
      + ["Omzet"]
      + [f"Bagi Hasil {k}" for k in KATEGORI_ORDER]
      + ["BH", "Flat", "Selisih", "Efektif %"]
  )
  out = out[urut].rename(
      columns={
          "TEKNISI": "Nama Teknisi",
          "CABANG": "Cabang",
          "Omzet": "Omzet Jasa (Total)",
          "BH": "Bagi Hasil (Aturan)",
          "Flat": lbl_flat,
      }
  )
  return out.sort_values("Bagi Hasil (Aturan)", ascending=False).reset_index(
      drop=True
  )
...
def _tulis_sheet(wb, df, nama_sheet, judul, kolom_gaji=False):
  ...
  ws.cell(row=1, column=1, value=judul).font = Font(
  ...
  kol_rp = [
      c
      for c in df.columns
      if c.startswith(("Omzet", "Bagi Hasil"))
      or c in (lbl_flat, "Selisih")
      or c in KOLOM_GAJI
  ]
  idx_rp = [df.columns.get_loc(c) + 1 for c in kol_rp]
  idx_baris = (
      (df.columns.get_loc("Baris") + 1) if "Baris" in df.columns else None
  )
  idx_pct = (
      (df.columns.get_loc("Efektif %") + 1)
      if "Efektif %" in df.columns
      else None
  )
  ...
  for i in range(n_baris):
    ...
    for k in KATEGORI_ORDER:
      ...
      if omzet_k in df.columns and bh_k in df.columns:
        ...
        ws.cell(
            row=r,
            column=df.columns.get_loc(bh_k) + 1,
    ...
    if "Omzet Jasa (Total)" in df.columns and "Omzet Interface" in df.columns:
      ws.cell(
          row=r,
          column=df.columns.get_loc("Omzet Jasa (Total)") + 1,
          value=(
              f"=SUM({kol_letter('Omzet Interface')}{r}:{kol_letter('Omzet' + ' Lainnya')}{r})"
          ),
      )

    if (
        "Bagi Hasil (Aturan)" in df.columns
        and "Bagi Hasil Interface" in df.columns
    ):
      ws.cell(
          row=r,
          column=df.columns.get_loc("Bagi Hasil (Aturan)") + 1,
          value=(
              f"=SUM({kol_letter('Bagi Hasil Interface')}{r}:{kol_letter('Bagi' + ' Hasil Lainnya')}{r})"
          ),
      )
    ...
    if lbl_flat in df.columns and "Omzet Jasa (Total)" in df.columns:
      ws.cell(
          row=r,
          column=df.columns.get_loc(lbl_flat) + 1,
          value=f"={kol_letter('Omzet Jasa (Total)')}{r}*{tarif_flat/100}",
      )

    if (
        "Selisih" in df.columns
        and "Bagi Hasil (Aturan)" in df.columns
        and lbl_flat in df.columns
    ):
      ws.cell(
          row=r,
          column=df.columns.get_loc("Selisih") + 1,
          value=(
              f"={kol_letter('Bagi Hasil (Aturan)')}{r}-{kol_letter(lbl_flat)}{r}"
          ),
      )

    if (
        "Efektif %" in df.columns
        and "Omzet Jasa (Total)" in df.columns
        and "Bagi Hasil (Aturan)" in df.columns
    ):
      ws.cell(
          row=r,
          column=df.columns.get_loc("Efektif %") + 1,
          value=(
              f"=IF({kol_letter('Omzet Jasa (Total)')}{r}=0,0,{kol_letter('Bagi' + ' Hasil (Aturan)')}{r}/{kol_letter('Omzet' + ' Jasa (Total)')}{r}*100)"
          ),
      )
...
def _tulis_sheet_final(wb, df_teknisi, periode_str="PERIODE BERJALAN"):
  """Membuat Sheet 4 (FINAL) yang terhubung langsung secara dinamis ke Sheet 3 (RAW)."""
  ...
  ws.cell(
      row=2,
      column=2,
      value="DAFTAR BAGI HASIL MFLASH PT MADINAH MULTIMEDIA TELEKOMUNIKASI",
  ).font = Font(bold=True, size=11)
  ws.cell(
      row=3, column=2, value="TEKNISI HANDPHONE & LAPTOP"
  ).font = Font(bold=True, size=10)
  ws.cell(row=4, column=2, value="PERIODE:").font = Font(bold=True)
  ws.cell(row=4, column=4, value=periode_str).font = Font(bold=True)
  ...
  headers_final = [
      ...
      "DATA REKENING",
      "STATUS KARYAWAN",
      "TOTAL LABA KOTOR JASA SERVICE HP",
  ...
  for j, h in enumerate(headers_final, start=2):
  ...
  n_teknisi = len(df_teknisi)
  for i in range(n_teknisi):
    r_raw = 5 + i
    r_fin = 8 + i

    ws.cell(row=r_fin, column=2, value=i + 1)  # NO
    ws.cell(row=r_fin, column=4, value=f"=RAW!A{r_raw}")  # NAMA
    ws.cell(row=r_fin, column=5, value="Teknisi")  # JABATAN
    ws.cell(
        row=r_fin, column=6, value=f"=RAW!B{r_raw}"
    )  # LOKASI KERJA (dinamis dari Cabang di RAW)

    # Bank, No. Rek., A/n, DATA REKENING dikosongkan untuk pengisian manual
    ws.cell(row=r_fin, column=7, value=None)  # Bank
    ws.cell(row=r_fin, column=8, value=None)  # No. Rek.
    ws.cell(row=r_fin, column=9, value=None)  # A/n
    ws.cell(row=r_fin, column=10, value=None)  # DATA REKENING

    ws.cell(row=r_fin, column=11, value="MITRA")  # STATUS KARYAWAN

    ws.cell(row=r_fin, column=12, value=f"=RAW!I{r_raw}")  # LABA KOTOR
    ws.cell(row=r_fin, column=13, value=f"=RAW!P{r_raw}")  # PENERIMAAN BAGI HASIL
    ws.cell(row=r_fin, column=14, value=f"=RAW!AD{r_raw}")  # PENDAPATAN LAIN-LAIN
    ws.cell(
        row=r_fin, column=15, value=f"=SUM(M{r_fin}:N{r_fin})"
    )  # TOTAL PENDAPATAN

    # Potongan T..AB di RAW -> P..X di FINAL
    pot_cols_raw = ["T", "U", "V", "W", "X", "Y", "Z", "AA", "AB"]
    for p_idx, c_let in enumerate(pot_cols_raw):
      ws.cell(row=r_fin, column=16 + p_idx, value=f"=RAW!{c_let}{r_raw}")

    ws.cell(row=r_fin, column=25, value=f"=RAW!AC{r_raw}")  # Total Potongan
    ws.cell(row=r_fin, column=26, value=f"=O{r_fin}-Y{r_fin}")  # BAGI HASIL NETT

    # Formula Informasi
    info = (
        '=_xlfn.TEXTJOIN(" | ",TRUE,IF(V'
        f'{r_fin}>0,"Biaya Pendaftaran Koperasi: "&TEXT(V{r_fin},"#.##0"),""),IF(W{r_fin}>0,"Simpanan'
        f' Pokok: "&TEXT(W{r_fin},"#.##0"),""),IF(X{r_fin}>0,"Simpanan Wajib:'
        f' "&TEXT(X{r_fin},"#.##0"),""))'
    )
    ws.cell(row=r_fin, column=27, value=info)

    # Formatting
    for col_idx in range(12, 27):
      ws.cell(row=r_fin, column=col_idx).number_format = "#,##0"
      ws.cell(row=r_fin, column=col_idx).border = Border(bottom=thin)

  # Baris Total
  if n_teknisi > 0:
    r_tot = 8 + n_teknisi
    ws.cell(row=r_tot, column=4, value="TOTAL").font = Font(bold=True)
    for c_idx in range(12, 27):
      col_let = get_column_letter(c_idx)
      cell = ws.cell(
          row=r_tot, column=c_idx, value=f"=SUM({col_let}8:{col_let}{r_tot-1})"
      )
      cell.font = Font(bold=True)
      cell.number_format = "#,##0"
      cell.fill = PatternFill("solid", fgColor="DCE6F1")
      cell.border = Border(top=Side(style="medium", color="1F3864"))

  # Blok Tanda Tangan
  r_sign = 8 + n_teknisi + 3
  ws.cell(row=r_sign, column=4, value="Owner Madinah Group").font = Font(
      bold=True
  )
  ws.cell(row=r_sign, column=15, value="Corcomm").font = Font(bold=True)
  ws.cell(row=r_sign, column=19, value="Finance & Acc Manager").font = Font(
      bold=True
  )
  ws.cell(row=r_sign, column=25, value="HR Manager").font = Font(bold=True)
...
def buat_excel(df_sumber, raw_bytes=None):
  ...
  # Tulis Sheet FINAL (Sheet 4)
  if not d.empty:
    df_tek = rekap_kualifikasi(d, ["TEKNISI"])
    _tulis_sheet_final(wb, df_tek, periode_str=periode_txt)
...
if isinstance(pilih, tuple):
  ...
  label_tgl_file = (
      f"{a_f.day} {BULAN_NAMES[a_f.month]} - {b_f.day}"
      f" {BULAN_NAMES[b_f.month]} {b_f.year}"
  )
else:
  if not jasa_tampil.empty and "TGL" in jasa_tampil.columns:
    ...
    if pd.notna(t_min_f) and pd.notna(t_max_f):
      label_tgl_file = (
          f"{t_min_f.day} {BULAN_NAMES[t_min_f.month]} - {t_max_f.day}"
          f" {BULAN_NAMES[t_max_f.month]} {t_max_f.year}"
      )
    else:
...
with st.container():
  st.markdown("##### 📊 Unduh Excel (Sesuai Template + RAW & FINAL)")
  st.caption(
      "Mengunduh file Excel yang berisi **Sheet 1 (Rincian Faktur Penjualan)**, "
      "**Sheet 2 (Pivot)**, **Sheet 3 (RAW)**, dan **Sheet 4 (FINAL)**."
  )
  if st.button(
      "🧾 Siapkan berkas Excel", key="siap_xlsx", use_container_width=True
  ):
    with st.spinner("Menyusun workbook..."):
      st.session_state["xlsx_bytes"] = buat_excel(
          jasa_tampil, raw_uploaded_bytes
      )
